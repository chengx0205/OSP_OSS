...
public class AlarmLogHandler extends BaseLogger{
	...
	/*　根据新的日志修改规范必须传入流程Ｍａｐ参数（业务线）－－－由业务线找到应用名称信息　２０１６－０６－０２
	public void warn(String msgContext) {
		LogLocation logLocation = new LogLocation(new Throwable(), getClassname());
		String tmp = convAlarmMessage(LOG_LEVEL_WARNING, msgContext, logLocation.getLineNumber());
		logger.warn(tmp);
	...
	public void debug(String msgContext,Map<String, String> variableMap) {
		LogLocation logLocation = new LogLocation(new Throwable(), getClassname());
		String tmp = convAlarmMessage(LOG_LEVEL_INFO, msgContext, logLocation.getLineNumber(),variableMap);
		logger.debug(tmp);
	...
	public void error(String msgContext) {
		LogLocation logLocation = new LogLocation(new Throwable(), getClassname());
		String tmp = convAlarmMessage(LOG_LEVEL_ALARM_ERROR, msgContext, logLocation.getLineNumber());
		logger.error(tmp);
	}
	...
	private String convAlarmMessage(final String p_loglevel,final String msgContext,final String lineNumber) {
		...
		return convAlarmMessage(p_loglevel,msgContext,lineNumber,variableMap);
	}
